...
async def get_analytics_summary(
    ...
    try:
        ...
        # Fallback to last_reviewed_at if ReviewLog table is empty
        if not logs:
            for card in cards:
        ...
        total_reviews = len(logs) if logs else sum(date_counts.values())
        ...
        if cards:
            ...
            retention_score = (
                round((0.7 * ease_score) + (0.3 * reviewed_ratio))
                if reviewed_count > 0
                else 0
            )
        else:
